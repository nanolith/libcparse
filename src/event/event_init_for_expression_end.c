/**
 * \file src/event/event_init_for_expression_end.c
 *
 * \brief Initialize an event instance for an expression end event.
 *
    ...
    CPARSE_SYM(event)* ev, const CPARSE_SYM(cursor)* cursor)
{
    return
        event_init(
            ev, CPARSE_EVENT_TYPE_EXPRESSION_END, CPARSE_EVENT_CATEGORY_BASE,
            cursor);
}
