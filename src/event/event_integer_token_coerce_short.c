/**
 * \file src/event/event_integer_token_coerce_short.c
 *
 * \brief Perform a potentially lossy conversion to a short.
 *
    ...
    const CPARSE_SYM(event_integer_token)* ev)
{
    ...
    {
        return (short)ev->val.signed_val;
    }
    else
    {
        return (short)ev->val.unsigned_val;
    }
}
