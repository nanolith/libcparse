/**
 * \file src/event/event_init_for_token_comma.c
 *
 * \brief Initialize an event instance for the comma token event.
 *
    ...
    CPARSE_SYM(event)* ev, const CPARSE_SYM(cursor)* cursor)
{
    return
        event_init(
            ev, CPARSE_EVENT_TYPE_TOKEN_COMMA, CPARSE_EVENT_CATEGORY_BASE,
            cursor);
}
