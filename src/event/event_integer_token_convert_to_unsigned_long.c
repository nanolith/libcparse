/**
 * \file src/event/event_integer_token_convert_to_unsigned_long.c
 *
 * \brief Attempt to convert an integer token to an unsigned long.
 *
    ...
    unsigned long* val, const CPARSE_SYM(event_integer_token)* ev)
{
    if (event_integer_token_is_signed(ev))
    {
            ...
            if (ev->val.signed_val >= 0)
            {
                *val = (unsigned long)ev->val.signed_val;
                return STATUS_SUCCESS;
            }
        #else
            if (ev->val.signed_val <= ULONG_MAX && ev->val.signed_val >= 0)
            {
                *val = (unsigned long)ev->val.signed_val;
                return STATUS_SUCCESS;
            }
    ...
    {
        if (ev->val.unsigned_val <= ULONG_MAX)
        {
            *val = (unsigned long)ev->val.unsigned_val;
            return STATUS_SUCCESS;
        }
    }
