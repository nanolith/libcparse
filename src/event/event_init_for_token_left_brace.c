/**
 * \file src/event/event_init_for_token_left_brace.c
 *
 * \brief Initialize an event instance for the left brace token event.
 *
    ...
    CPARSE_SYM(event)* ev, const CPARSE_SYM(cursor)* cursor)
{
    return
        event_init(
            ev, CPARSE_EVENT_TYPE_TOKEN_LEFT_BRACE, CPARSE_EVENT_CATEGORY_BASE,
            cursor);
}
