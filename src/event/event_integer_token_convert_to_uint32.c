/**
 * \file src/event/event_integer_token_convert_to_uint32.c
 *
 * \brief Attempt to convert an integer token to an unsigned 32-bit integer.
 *
    ...
    uint32_t* val, const CPARSE_SYM(event_integer_token)* ev)
{
    ...
    {
        if (ev->val.signed_val <= UINT32_MAX && ev->val.signed_val >= 0)
        {
            *val = (uint32_t)ev->val.signed_val;
            return STATUS_SUCCESS;
        }
    }
    else
    {
        if (ev->val.unsigned_val <= UINT32_MAX)
        {
            *val = (uint32_t)ev->val.unsigned_val;
            return STATUS_SUCCESS;
        }
    }
