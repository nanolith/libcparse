/**
 * \file src/event/event_init_for_token_question.c
 *
 * \brief Initialize an event instance for the question token event.
 *
    ...
    CPARSE_SYM(event)* ev, const CPARSE_SYM(cursor)* cursor)
{
    return
        event_init(
            ev, CPARSE_EVENT_TYPE_TOKEN_QUESTION, CPARSE_EVENT_CATEGORY_BASE,
            cursor);
}
