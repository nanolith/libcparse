/**
 * \file src/event/event_init_for_token_mod_equal.c
 *
 * \brief Initialize an event instance for the mod equal token event.
 *
    ...
    CPARSE_SYM(event)* ev, const CPARSE_SYM(cursor)* cursor)
{
    return
        event_init(
            ev, CPARSE_EVENT_TYPE_TOKEN_MOD_EQUAL, CPARSE_EVENT_CATEGORY_BASE,
            cursor);
}
