/**
 * \file src/event/event_init_for_token_logical_and.c
 *
 * \brief Initialize an event instance for the logical and token event.
 *
    ...
    CPARSE_SYM(event)* ev, const CPARSE_SYM(cursor)* cursor)
{
    return
        event_init(
            ev, CPARSE_EVENT_TYPE_TOKEN_LOGICAL_AND, CPARSE_EVENT_CATEGORY_BASE,
            cursor);
}
