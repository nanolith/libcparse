/**
 * \file src/event/event_integer_token_convert_to_int16.c
 *
 * \brief Attempt to convert an integer token to a 16-bit integer.
 *
    ...
    int16_t* val, const CPARSE_SYM(event_integer_token)* ev)
{
    ...
    {
        if (ev->val.signed_val <= INT16_MAX && ev->val.signed_val >= INT16_MIN)
        {
            *val = (int16_t)ev->val.signed_val;
            return STATUS_SUCCESS;
        }
    }
    else
    {
        if (ev->val.unsigned_val <= INT16_MAX)
        {
            *val = (int16_t)ev->val.unsigned_val;
            return STATUS_SUCCESS;
        }
    }
