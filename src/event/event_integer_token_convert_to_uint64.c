/**
 * \file src/event/event_integer_token_convert_to_uint64.c
 *
 * \brief Attempt to convert an integer token to an unsigned 64-bit integer.
 *
    ...
    uint64_t* val, const CPARSE_SYM(event_integer_token)* ev)
{
    if (event_integer_token_is_signed(ev))
    {
            ...
            {
                *val = (uint64_t)ev->val.signed_val;
                return STATUS_SUCCESS;
            }
        #else
            if (ev->val.signed_val <= UINT64_MAX && ev->val.signed_val >= 0)
            {
                *val = (uint64_t)ev->val.signed_val;
                return STATUS_SUCCESS;
            }
    ...
    {
        if (ev->val.unsigned_val <= UINT64_MAX)
        {
            *val = (uint64_t)ev->val.unsigned_val;
            return STATUS_SUCCESS;
        }
    }
