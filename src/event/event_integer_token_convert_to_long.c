/**
 * \file src/event/event_integer_token_convert_to_long.c
 *
 * \brief Attempt to convert an integer token to a long.
 *
    ...
    long* val, const CPARSE_SYM(event_integer_token)* ev)
{
    ...
    {
        if (ev->val.signed_val <= LONG_MAX && ev->val.signed_val >= LONG_MIN)
        {
            *val = (long)ev->val.signed_val;
            return STATUS_SUCCESS;
        }
    }
    else
    {
        if (ev->val.unsigned_val <= LONG_MAX)
        {
            *val = (long)ev->val.unsigned_val;
            return STATUS_SUCCESS;
        }
    }
