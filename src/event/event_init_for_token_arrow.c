/**
 * \file src/event/event_init_for_token_arrow.c
 *
 * \brief Initialize an event instance for the arrow token event.
 *
    ...
    CPARSE_SYM(event)* ev, const CPARSE_SYM(cursor)* cursor)
{
    return
        event_init(
            ev, CPARSE_EVENT_TYPE_TOKEN_ARROW, CPARSE_EVENT_CATEGORY_BASE,
            cursor);
}
