/**
 * \file src/event/event_init_for_token_minus.c
 *
 * \brief Initialize an event instance for the minus token event.
 *
    ...
    CPARSE_SYM(event)* ev, const CPARSE_SYM(cursor)* cursor)
{
    return
        event_init(
            ev, CPARSE_EVENT_TYPE_TOKEN_MINUS, CPARSE_EVENT_CATEGORY_BASE,
            cursor);
}
