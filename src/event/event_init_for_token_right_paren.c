/**
 * \file src/event/event_init_for_token_right_paren.c
 *
 * \brief Initialize an event instance for the right paren token event.
 *
    ...
    CPARSE_SYM(event)* ev, const CPARSE_SYM(cursor)* cursor)
{
    return
        event_init(
            ev, CPARSE_EVENT_TYPE_TOKEN_RIGHT_PAREN, CPARSE_EVENT_CATEGORY_BASE,
            cursor);
}
