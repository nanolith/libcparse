/**
 * \file src/event/event_integer_token_convert_to_unsigned_long_long.c
 *
 * \brief Attempt to convert an integer token to an unsigned long long.
 *
    ...
    unsigned long long* val, const CPARSE_SYM(event_integer_token)* ev)
{
    if (event_integer_token_is_signed(ev))
    {
        ...
        {
            *val = (unsigned long long)ev->val.signed_val;
            return STATUS_SUCCESS;
        }
    }
    else
    {
        *val = (unsigned long long)ev->val.unsigned_val;
        return STATUS_SUCCESS;
    }
