/**
 * \file src/event/event_init_for_comment_line_end.c
 *
 * \brief Initialize an event instance for the comment line end event.
 *
    ...
    CPARSE_SYM(event)* ev, const CPARSE_SYM(cursor)* cursor)
{
    return
        event_init(
            ev, CPARSE_EVENT_TYPE_COMMENT_LINE_END, CPARSE_EVENT_CATEGORY_BASE,
            cursor);
}
