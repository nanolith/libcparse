/**
 * \file src/event/event_integer_token_convert_to_int8.c
 *
 * \brief Attempt to convert an integer token to an 8-bit integer.
 *
    ...
    int8_t* val, const CPARSE_SYM(event_integer_token)* ev)
{
    ...
    {
        if (ev->val.signed_val <= INT8_MAX && ev->val.signed_val >= INT8_MIN)
        {
            *val = (int8_t)ev->val.signed_val;
            return STATUS_SUCCESS;
        }
    }
    else
    {
        if (ev->val.unsigned_val <= INT8_MAX)
        {
            *val = (int8_t)ev->val.unsigned_val;
            return STATUS_SUCCESS;
        }
    }
