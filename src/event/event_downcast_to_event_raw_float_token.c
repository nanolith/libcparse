/**
 * \file src/event/event_downcast_to_event_raw_float_token.c
 *
 * \brief Attempt to convert an \ref event to an \ref event_raw_float_token.
 *
...
{
    /* verify that this is a raw float token event. */
    if (CPARSE_EVENT_TYPE_TOKEN_VALUE_RAW_FLOAT != event_get_type(ev))
    {
        return ERROR_LIBCPARSE_BAD_CAST;
    }

    /* perform the downcast. */
    *s_ev = (event_raw_float_token*)ev;
