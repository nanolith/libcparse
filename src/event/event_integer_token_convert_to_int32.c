/**
 * \file src/event/event_integer_token_convert_to_int32.c
 *
 * \brief Attempt to convert an integer token to a 32-bit integer.
 *
    ...
    int32_t* val, const CPARSE_SYM(event_integer_token)* ev)
{
    ...
    {
        if (ev->val.signed_val <= INT32_MAX && ev->val.signed_val >= INT32_MIN)
        {
            *val = (int32_t)ev->val.signed_val;
            return STATUS_SUCCESS;
        }
    }
    else
    {
        if (ev->val.unsigned_val <= INT32_MAX)
        {
            *val = (int32_t)ev->val.unsigned_val;
            return STATUS_SUCCESS;
        }
    }
