/**
 * \file src/event/event_integer_token_convert_to_short.c
 *
 * \brief Attempt to convert an integer token to a short.
 *
    ...
    short* val, const CPARSE_SYM(event_integer_token)* ev)
{
    ...
    {
        if (ev->val.signed_val <= SHRT_MAX && ev->val.signed_val >= SHRT_MIN)
        {
            *val = (short)ev->val.signed_val;
            return STATUS_SUCCESS;
        }
    }
    else
    {
        if (ev->val.unsigned_val <= SHRT_MAX)
        {
            *val = (short)ev->val.unsigned_val;
            return STATUS_SUCCESS;
        }
    }
