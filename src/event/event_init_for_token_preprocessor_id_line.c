/**
 * \file src/event/event_init_for_token_preprocessor_id_line.c
 *
 * \brief Initialize an event instance for the preprocessor id line token
 * event.
 *
    ...
    CPARSE_SYM(event)* ev, const CPARSE_SYM(cursor)* cursor)
{
    return
        event_init(
            ev, CPARSE_EVENT_TYPE_TOKEN_PP_ID_LINE, CPARSE_EVENT_CATEGORY_BASE,
            cursor);
}
