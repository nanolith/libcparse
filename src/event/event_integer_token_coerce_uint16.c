/**
 * \file src/event/event_integer_token_coerce_uint16.c
 *
 * \brief Perform a potentially lossy conversion to a uint16.
 *
    ...
    const CPARSE_SYM(event_integer_token)* ev)
{
    ...
    {
        return (uint16_t)ev->val.signed_val;
    }
    else
    {
        return (uint16_t)ev->val.unsigned_val;
    }
}
