/**
 * \file src/event/event_init_for_token_minus_equal.c
 *
 * \brief Initialize an event instance for the minus equal token event.
 *
    ...
    CPARSE_SYM(event)* ev, const CPARSE_SYM(cursor)* cursor)
{
    return
        event_init(
            ev, CPARSE_EVENT_TYPE_TOKEN_MINUS_EQUAL, CPARSE_EVENT_CATEGORY_BASE,
            cursor);
}
