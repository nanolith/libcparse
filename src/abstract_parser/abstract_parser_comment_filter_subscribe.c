/**
 * \file
 * src/abstract_parser/abstract_parser_comment_filter_subscribe.c
 *
 * \brief Send a subscription request to the \ref comment_filter.
 *
    ...
    CPARSE_SYM(abstract_parser)* ap, CPARSE_SYM(event_handler)* eh)
{
    ...
    retval = message_subscribe_init_for_comment_filter(&msg, eh);
