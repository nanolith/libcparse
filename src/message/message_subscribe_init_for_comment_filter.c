/**
 * \file src/message/message_subscribe_init_for_comment_filter.c
 *
 * \brief \ref message_subscribe type init method for comment filter
 * subscriptions.
 *
    ...
    CPARSE_SYM(message_subscribe)* msg, CPARSE_SYM(event_handler)* handler)
{
    return
        message_subscribe_init(
            msg, CPARSE_MESSAGE_TYPE_COMMENT_FILTER_SUBSCRIBE, handler);
}
