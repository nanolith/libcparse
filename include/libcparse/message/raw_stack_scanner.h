 ...
 */
int FN_DECL_MUST_CHECK CPARSE_SYM(message_rss_add_input_stream_init)(
 ...
 *      - a non-zero error code on failure.
 */
int FN_DECL_MUST_CHECK CPARSE_SYM(message_rss_add_input_stream_dispose)(
...
#define __INTERNAL_CPARSE_IMPORT_message_raw_stack_scanner_sym(sym) \
    ...
    static inline int FN_DECL_MUST_CHECK \
    sym ## message_rss_add_input_stream_dispose( \
    ...
    sym ## message_rss_add_input_stream_upcast( \
        CPARSE_SYM(message_rss_add_input_stream)* x) { \
