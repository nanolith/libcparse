 ...
 *      - a non-zero error code on failure.
 */
int FN_DECL_MUST_CHECK CPARSE_SYM(message_subscribe_init_for_comment_scanner)(
    CPARSE_SYM(message_subscribe)* msg, CPARSE_SYM(event_handler)* handler);
 ...
 *      - a non-zero error code on failure.
 */
int FN_DECL_MUST_CHECK CPARSE_SYM(message_subscribe_init_for_line_wrap_filter)(
    CPARSE_SYM(message_subscribe)* msg, CPARSE_SYM(event_handler)* handler);
 ...
 *      - a non-zero error code on failure.
 */
int FN_DECL_MUST_CHECK CPARSE_SYM(message_subscribe_init_for_raw_stack_scanner)(
    CPARSE_SYM(message_subscribe)* msg, CPARSE_SYM(event_handler)* handler);
...
#define __INTERNAL_CPARSE_IMPORT_message_subscription_sym(sym) \
    ...
    static inline int FN_DECL_MUST_CHECK \
    sym ## message_subscribe_init_for_comment_scanner(\
        CPARSE_SYM(message_subscribe)* x, CPARSE_SYM(event_handler)* y) { \
            return \
                CPARSE_SYM(message_subscribe_init_for_comment_scanner)(x,y); } \
    static inline int FN_DECL_MUST_CHECK \
    sym ## message_subscribe_init_for_line_wrap_filter(\
        CPARSE_SYM(message_subscribe)* x, CPARSE_SYM(event_handler)* y) { \
            return \
                CPARSE_SYM(message_subscribe_init_for_line_wrap_filter)( \
                    x,y); } \
    static inline int FN_DECL_MUST_CHECK \
    sym ## message_subscribe_init_for_nl_ws_filter(\
        CPARSE_SYM(message_subscribe)* x, CPARSE_SYM(event_handler)* y) { \
            return \
        ...
        CPARSE_SYM(message_subscribe)* x, CPARSE_SYM(event_handler)* y) { \
            return \
                CPARSE_SYM(message_subscribe_init_for_preprocessor_scanner)( \
                    x,y); } \
    static inline int FN_DECL_MUST_CHECK \
    sym ## message_subscribe_init_for_rflo_filter(\
        CPARSE_SYM(message_subscribe)* x, CPARSE_SYM(event_handler)* y) { \
            return \
                CPARSE_SYM(message_subscribe_init_for_rflo_filter)(x,y); } \
    static inline int FN_DECL_MUST_CHECK \
    sym ## message_subscribe_init_for_raw_stack_scanner(\
        CPARSE_SYM(message_subscribe)* x, CPARSE_SYM(event_handler)* y) { \
            return \
    ...
    static inline int FN_DECL_MUST_CHECK \
    sym ## message_downcast_to_message_subscribe( \
