...
#define __INTERNAL_CPARSE_IMPORT_event_raw_string_sym(sym) \
    ...
    static inline const char* sym ## event_raw_string_token_get( \
        const CPARSE_SYM(event_raw_string_token)* x) { \
            return CPARSE_SYM(event_raw_string_token_get)(x); } \
    static inline int FN_DECL_MUST_CHECK \
    sym ## event_raw_string_token_convert( \
        CPARSE_SYM(event_string)* x, \
        const CPARSE_SYM(event_raw_string_token)* y) { \
            return CPARSE_SYM(event_raw_string_token_convert)(x,y); } \
    static inline int FN_DECL_MUST_CHECK \
    sym ## event_downcast_to_event_raw_string_token( \
        CPARSE_SYM(event_raw_string_token)** x, CPARSE_SYM(event)* y) { \
            return \
                CPARSE_SYM(event_downcast_to_event_raw_string_token)(x,y); } \
    static inline CPARSE_SYM(event)* sym ## event_raw_string_token_upcast( \
        CPARSE_SYM(event_raw_string_token)* x) { \
            return CPARSE_SYM(event_raw_string_token_upcast)(x); } \
    CPARSE_END_EXPORT \
