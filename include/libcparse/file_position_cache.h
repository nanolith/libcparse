 ...
 *      - a non-zero error code on failure.
 */
int FN_DECL_MUST_CHECK CPARSE_SYM(file_position_cache_position_get)(
    CPARSE_SYM(file_position_cache)* cache,
    const CPARSE_SYM(cursor)** position);
 ...
 *      - a non-zero error code on failure.
 */
int FN_DECL_MUST_CHECK CPARSE_SYM(file_position_cache_raw_character_broadcast)(
 ...
 *      - a non-zero error code on failure.
 */
int FN_DECL_MUST_CHECK CPARSE_SYM(file_position_cache_newline_token_broadcast)(
    CPARSE_SYM(file_position_cache)* cache, CPARSE_SYM(event_reactor)* reactor);
 ...
 */
int FN_DECL_MUST_CHECK
CPARSE_SYM(file_position_cache_final_newline_token_broadcast)(
    CPARSE_SYM(file_position_cache)* cache, CPARSE_SYM(event_reactor)* reactor);
...
#define __INTERNAL_CPARSE_IMPORT_file_position_cache_sym(sym) \
    ...
    static inline int FN_DECL_MUST_CHECK sym ## file_position_cache_release( \
        CPARSE_SYM(file_position_cache)* x) { \
            return CPARSE_SYM(file_position_cache_release)(x); } \
    static inline int FN_DECL_MUST_CHECK sym ## file_position_cache_set( \
    ...
    static inline int FN_DECL_MUST_CHECK sym ## file_position_cache_file_get( \
        CPARSE_SYM(file_position_cache)* x, const char** y) { \
    ...
    static inline int FN_DECL_MUST_CHECK \
    sym ## file_position_cache_raw_character_broadcast( \
        ...
        CPARSE_SYM(file_position_cache)* x, CPARSE_SYM(event_reactor)* y) { \
            return \
                CPARSE_SYM(file_position_cache_whitespace_token_broadcast)( \
                    x,y); } \
    static inline int FN_DECL_MUST_CHECK \
    sym ## file_position_cache_newline_token_broadcast( \
        CPARSE_SYM(file_position_cache)* x, CPARSE_SYM(event_reactor)* y) { \
            return \
                CPARSE_SYM(file_position_cache_newline_token_broadcast)(x,y); }\
    static inline int \
    sym ## file_position_cache_final_newline_token_broadcast( \
        CPARSE_SYM(file_position_cache)* x, CPARSE_SYM(event_reactor)* y) { \
            return \
                CPARSE_SYM(file_position_cache_final_newline_token_broadcast)( \
                    x,y); }\
